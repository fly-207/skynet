...
struct socket
{
	...
	ATOM_INT type;			 // 使用的协议名称TCP or UDP
	uint8_t protocol;		 // socket的类型, 包括 SOCKET_TYPE_PLISTEN, SOCKET_TYPE_PACCEPT, SOCKET_TYPE_CONNECTING, SOCKET_TYPE_CONNECTED 等
	bool reading;
	bool writing;
	bool closing;
	ATOM_INT udpconnecting; // udp相关成员, 本文不讨论udp相关的内容
	...
	struct spinlock dw_lock; // 从1.2开始, 是skynet就添加了多线程处理机制, 如果发送队列high和low list都为空的情况下, 且sending标记未被设置, 则直接通过worker线程直接发送,
							 // 这个dw_lock的作用, 就是为了避免worker线程和socket线程, 同时对同一个socket实例的send buffer进行write操作导致严重的bug
	int dw_offset;			 // 在worker线程中, 如果不能将buffer一次写完, skynet会将这块buffer的地址放在dw_buffer中, 而dw_offset标记的是, 已经写了多少字节
...
struct socket_server
{
	...
	ATOM_INT alloc_id;					// 用于生成skynet socket实例id的变量, 每次生成一个socket实例, 就自增1(溢出前会回绕),
										// socket实例 id是根据当前分配的 alloc_id hash 计算得出
	int event_n;						// 每次有IO准备就绪时, IO事件的总数
	int event_index;					// 当前处理到哪个epoll事件的索引
	struct socket_object_interface soi; // 当发送自定义结构的userobject时, 需要应用层注册, 自己要使用的内存分配函数和释放函数
	struct event ev[MAX_EVENT];			// 当epoll实例中的interest list中, 有fd的IO准备就绪时, 响应的事件信息, 就会被填写到这个结构中
	struct socket slot[MAX_SOCKET];		// 在skynet中, 每个系统socket被创建出来的时候, 都会有一个skynet的 socket实例与之对应, skynet最多可以支持65536(2^16)个socket
	char buffer[MAX_INFO];				// 用于临时存放, 状态信息的buffer
	uint8_t udpbuffer[MAX_UDP_PACKAGE]; // UDP相关, 本文不讨论UDP相关的内容
	fd_set rfds;						// 传给select函数的参数, 主要用于检查, worker线程是否有消息发送给socket线程
};
