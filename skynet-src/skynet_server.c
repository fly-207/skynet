 ...
 * 分发消息到指定的上下文中。
 *
 * @param ctx 上下文结构体，包含处理消息所需的各种状态和回调函数。
 * @param msg 消息结构体，包含消息的来源、类型、会话ID、数据大小和实际数据。
 */
static void
dispatch_message(struct skynet_context *ctx, struct skynet_message *msg)
{
	...
	// 开始检查调用上下文是否正确
	CHECKCALLING_BEGIN(ctx)
	// 设置当前线程的特定关键字值，通常用于标识当前的上下文句柄
	pthread_setspecific(G_NODE.handle_key, (void *)(uintptr_t)(ctx->handle));
	// 从消息大小中解析出消息类型和实际数据大小
 ...
 * 分发消息到指定的服务上下文。
 *
 * @param sm 监控接口，用于监控消息处理的过程。
 * @param q 指定的消息队列，如果为NULL，则尝试从全局消息队列中获取。
 * @param weight 当从消息队列中取出消息时，根据这个权重参数调整一次取出的消息数量。
 * @return 返回下一个需要处理的消息队列，如果没有则返回NULL。
 */
struct message_queue *
skynet_context_message_dispatch(struct skynet_monitor *sm, struct message_queue *q, int weight)
{
	// 如果指定的消息队列为空，尝试从全局消息队列获取
	if (q == NULL)
	{
	...
	if (ctx == NULL)
	{
		// 如果获取不到上下文，说明服务已经被销毁，释放消息队列并尝试获取下一个全局消息队列
		struct drop_t d = {handle};
	...
	// 断言当前消息队列与上下文关联的消息队列一致
	assert(q == ctx->queue);
	// 尝试从全局消息队列获取下一个需要处理的消息队列
	struct message_queue *nq = skynet_globalmq_pop();
	if (nq)
	{
		// 如果存在下一个消息队列，将当前队列放回全局队列并返回下一个队列
