 ...
 * 计算MD5消息摘要的核心函数。
 *
 * @param w 存储16个32位字的数组，基于输入消息的异或运算结果。
 * @param result 存储最终4个32位字的消息摘要的数组。
 *
 * 该函数通过64步迭代压缩函数来更新消息摘要的四个32位字a、b、c和d。
 * 在每一步中，根据当前的迭代次数i、输入字w[i]和几个固定的常量k[i]进行运算。
 * 迭代过程中使用了不同的逻辑函数f和位移函数g，以确保覆盖不同的位操作。
 * 最后，将更新后的a、b、c和d存储在result数组中。
 */
static void
...
/**
 * hmac64函数使用MD5算法进行哈希计算，但不使用填充(padding)。
 * 结果是(c^d .. a^b)的形式，其中c、d、a、b是哈希计算过程中的中间值。
 *
 * @param x 输入的两个32位整数数组，代表第一部分密钥。
 * @param y 输入的两个32位整数数组，代表第二部分密钥。
 * @param result 存储计算结果的两个32位整数数组。
 */
static void
hmac(uint32_t x[2], uint32_t y[2], uint32_t result[2])
{
	uint32_t w[16]; // 用于存储中间计算结果的数组
	uint32_t r[4];	// 存储MD5哈希算法的最终结果
	int i;

	// 循环中将x和y的值按特定方式填充到w数组中，为MD5计算做准备
	for (i = 0; i < 16; i += 4)
	{
		w[i] = x[1];
		w[i + 1] = x[0];
	...
	// 调用digest_md5函数，对w数组进行MD5哈希计算，结果存入r数组
	digest_md5(w, r);

	// 计算最终结果，通过异或操作得到result数组的值
	result[0] = r[2] ^ r[3];
	result[1] = r[0] ^ r[1];
}
...
/**
 * 将Lua栈上的两个64位无符号整数字符串转换为uint32_t数组。
 *
 * @param L lua_State的指针，表示当前的Lua环境。
 * @param xx 一个包含两个元素的uint32_t数组，用于存储转换后的第一个64位整数。
 * @param yy 一个包含两个元素的uint32_t数组，用于存储转换后的第二个64位整数。
 *
 * 函数首先从Lua栈中获取两个字符串，并验证它们的长度是否为8（因为要表示64位无符号整数）。
 * 如果长度不正确，则向Lua环境中抛出错误。之后，函数将这两个字符串转换为64位的无符号整数，
 * 并分别存储到xx和yy数组中。
 */
static void
...
/**
 * 计算64位HMAC（Hash-based Message Authentication Code）
 *
 * @param L Lua状态机，用于读取和返回计算结果
 * @return 返回1，将计算结果作为64位有符号整数推送到Lua栈上
 */
static int
lhmac64(lua_State *L)
{
