...
-- 创建一个新的协程。
-- @param f 一个函数，作为新协程的入口函数。
-- @return 返回新创建的协程的协程对象。
local function co_create(f)
    -- 尝试从协程池中获取一个协程对象，如果协程池为空，则创建一个新的协程
    ...
    if co == nil then
        ...
        co = coroutine_create(function(...)
            f(...)
            while true do
                -- 检查协程是否关联了session，如果是，则在退出时记录错误信息，提示可能未响应session
                local session = session_coroutine_id[co]
                if session and session ~= 0 then
                    local source = debug.getinfo(f, "S")
                    skynet.error(string.format("Maybe forgot response session %s from %s : %s:%d", session,
                        skynet.address(session_coroutine_address[co]), source.source, source.linedefined))
                end
                
                -- 协程退出时的清理工作，包括清除跟踪标签和session信息
...
function skynet.exit()
    fork_queue = {
        h = 1,
        t = 0
    } -- no fork coroutine can be execute after skynet.exit
...
-- 原始消息分发函数
-- prototype: 消息类型原型，决定消息的处理方式
-- msg: 消息体
-- sz: 消息体大小
-- session: 会话ID，标识发送者和接收者之间的通信
-- source: 消息来源，通常是发送者的服务地址
local function raw_dispatch_message(prototype, msg, sz, session, source)
    ...
    if prototype == 1 then
        ...
        if f then
            ...
            -- 暂停当前协程，并尝试恢复它，传递会话、来源和解析后的消息体
            suspend(co, coroutine_resume(co, session, source, p.unpack(msg, sz)))
        else
            -- 如果没有指定的处理函数，清除跟踪信息并根据会话ID发送错误消息或调用未知请求处理函数
            trace_source[source] = nil
            if session ~= 0 then
                c.send(source, skynet.PTYPE_ERROR, session, "")
            else
...
function skynet.dispatch_message(...)
    ...
    while true do
        ...
        if not fork_succ then
            -- 如果raw_dispatch_message调用成功，则将当前错误设置为fork_err；
            -- 否则，将当前错误信息追加到原有错误信息后。
            if succ then
                succ = false
